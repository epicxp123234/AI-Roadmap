...
// Static canned content for demo mode. Demo visitors have no auth session,
// so the AI endpoints (which require a JWT) will always 401 for them — see
// askClaude(). Rather than opening ask-doubt up to anonymous traffic (which
// would undo the auth/rate-limit hardening), demo mode just never calls it.
// This also makes the demo instant and immune to flaky mobile networks,
// which matters a lot the one time it's a stranger's first impression.
const DEMO_LECTURES=[
  {num:1,title:"Nobody Wants Your Idea (Yet)",hook:"You think you have a great business idea. Statistically, you're wrong — but stick with me.",coreIdea:"A business idea isn't valuable until it solves a problem someone will actually pay to make go away. Most first ideas are solutions looking for a problem, which is backwards and exactly why they flop.",example:"Think of every 'Uber for X' pitch from 2015. Cool tech, zero people who actually had the problem it 'solved.'",action:"Write down one problem you personally complain about at least once a week. That's your starting point, not your genius idea.",mistake:"Falling in love with your solution before you've confirmed anyone has the problem. Founders do this constantly — it's basically a rite of passage into failure.",takeaway:"Problems are worth more than ideas. Ideas are cheap; problems people will pay to fix are not.",cliffhanger:"Next up: how to figure out if anyone actually cares about that problem — before you waste six months building for an audience of nobody."},
  {num:2,title:"Talk To Humans, It's Uncomfortable But Required",hook:"You cannot validate a business from inside your own head. I don't care how smart you are.",coreIdea:"Market research isn't a survey you send to your mom. It's uncomfortable conversations with strangers who have zero incentive to be nice to you about your idea.",example:"Ten honest 'meh' reactions from strangers beat one enthusiastic 'this is so cool!' from your best friend.",action:"Find 3 people outside your friend group who might have the problem you picked. Ask them about their problem, not your solution.",mistake:"Asking leading questions like 'wouldn't you love an app that does X?' — congratulations, you just told them what to say.",takeaway:"If you can't get a stranger to complain about the problem unprompted, you don't have a problem worth solving yet.",cliffhanger:"Coming up: what to actually do with all this scattered feedback so it doesn't just sit in a notes app forever."},
  {num:3,title:"Your Competitors Are Doing You a Favor",hook:"Bad news: competitors exist. Good news: that means the market is real and you don't have to convince anyone it exists.",coreIdea:"Competition proves demand. The real danger isn't competitors — it's discovering zero competitors, which usually means zero market, not zero smart people.",example:"Every big food delivery app has three competitors doing basically the same thing in the same city. The market didn't collapse — it grew.",action:"List 2-3 existing options people currently use to deal with your target problem, even bad workarounds like a messy spreadsheet.",mistake:"Treating 'no direct competitors' as a green light instead of a red flag worth investigating.",takeaway:"You don't need to be the only option. You need to be the option people actually prefer.",cliffhanger:"Next: turning your basic idea into something that has an actual shot at getting built without a huge budget."},
  {num:4,title:"Build the Ugly Version First",hook:"Your business does not need a logo yet. It needs proof anyone cares.",coreIdea:"A Minimum Viable Product is the smallest, ugliest version of your idea that still tests the real assumption — that someone will use or pay for it.",example:"Airbnb's first version was two guys renting air mattresses on their own apartment floor. No app, no branding, just a real transaction.",action:"Describe the absolute smallest version of your idea that could get one real person to say yes to using it this week.",mistake:"Spending three months perfecting a logo and app icon before a single real customer exists. Nobody cares about your color palette yet.",takeaway:"Ship embarrassingly early. If you're not a little embarrassed by version one, you waited too long.",cliffhanger:"Last one today — and this one's the gut check."},
  {num:5,title:"The Money Question You're Avoiding",hook:"'We'll figure out monetization later' is how good ideas quietly die.",coreIdea:"A business idea without a believable path to revenue is a hobby, not a business. You don't need the exact number yet — you need a plausible story for who pays and why.",example:"Free-to-use apps that 'figure out revenue later' usually either never do, or bolt on ads nobody asked for and lose the users they had.",action:"Write one sentence: who pays you, and why is the amount worth it to them.",mistake:"Confusing 'people like this' with 'people will pay for this.' Those are very different sentences.",takeaway:"If you can't say who pays and why in one sentence, you're not ready to build yet — and that's fine, that's what this week was for.",homework:["Write down your one real problem, your one-sentence revenue story, and one stranger's honest reaction to both."]},
];
const DEMO_DOUBT_ANSWER="Good question — but heads up, you're in demo mode right now, so I'm not running live AI answers here (that's reserved for signed-in students so it stays fast and reliable for everyone). Sign up free and I'll actually answer this one properly, tailored to whatever you're learning.";
...
function Learn({ progress, roadmap, onUpdateProgress, user, isDemo }) {
  ...
  const loadLectures=useCallback(async()=>{
    ...
    if(isDemo){setLectures(DEMO_LECTURES);setLoading(false);return;}
  ...
  const submitDoubt=async()=>{
    ...
    setLoadingDoubt(true);setAnswer("");
    if(isDemo){setAnswer(DEMO_DOUBT_ANSWER);setLoadingDoubt(false);return;}
    try{const res=await askClaude([{role:"user",content:`${PROFESSOR_SYSTEM}\n\nStudent is learning "${roadmap.title}", this week: "${weekTopic}". Question: "${doubt}"\n\nAnswer clearly and specifically. Under 150 words.`}]);setAnswer(res||"No response. Try again.");}catch{setAnswer("Something went wrong.");}
